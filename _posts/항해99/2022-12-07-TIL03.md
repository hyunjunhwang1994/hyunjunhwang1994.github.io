...
시간이 좀 남아 플라스크에서의 세션에 대해서 공부해 보았다.

쿠키, 세션, JWT(Json Web Token)

쿠키의 경우 보안 상 이유로 보안이 필요한 서비스에는 잘 사용하지 않고,
세션의 경우에는 많이 사용되지만, 아무래도 서버에 저장하는 만큼 사용자의 수가  

많을 경우 서버에 부담이 될 수도 있다고 한다.

JWT는?
데이터 위변조를 막을 수도있고,
인증 정보에대한 별도의 서버 저장소가 필요하지도 않다고 한다.

다만 토큰자체의 길이가 길어 인증 요청이 많아질수록 네트워크 자체의 부하가 심해진다.

[참조 블로그](https://tecoble.techcourse.co.kr/post/2021-05-22-cookie-session-jwt/)