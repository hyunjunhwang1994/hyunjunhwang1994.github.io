...
[사진 출처및 참고 블로그](https://suhwan.dev/2019/02/24/jpa-vs-hibernate-vs-spring-data-jpa/)
...
일단은 Spring, H2, Mysql 쿼리형식을 이용 해
쿼리를 이용하는 방법부터 기술하겠습니다.

![](../../images/2022-12-24-09-37-25.png)

아라처럼 추가해 프로젝트를 생성해줍니다.
...
Connection type을 눌러,  
인메모리 방식으로 변경후 아래url을 url에 추가해주고 생성합니다.

 

jdbc:h2:mem:test;MODE=MYSQL;  
혹시 위의 url로 진행이안되거나 진행중 오류가 난다면,
...
그다음 테스트 커넥션을 누르고,  
만약 첫 실행시 드라이버 설치관련 문구가 나온다면 download driver files를 눌러 설치해주세요.




그럼 드라이버 설치를 자동으로해주고 아래처럼 성공 결과가 나옵니다.
...
MY SQL이 아닌 H2 쿼리문으로 인식해서 그런거같은데
사용하는데는 지장이없습니다!


쿼리문을 테스트할 때에는 해당 부분을 드래그 해준뒤 실행해야합니다.
