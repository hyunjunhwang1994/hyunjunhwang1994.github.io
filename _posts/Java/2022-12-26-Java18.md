...
- ⭐️자바의 모~든 클래스는 Object 클래스의 자식 클래스이다.
...
```java
        ...
        // 객체에서 String.equals가 데이터 비교로 사용되는 것 처럼
        // equals와, hashcode를 재정의 해주면?
...
재정의한 equals를 통해 필드값(데이터)을 비교하여 true & false를 리턴받게되고 , (원래같은 경우 해쉬 주소값만 비교합니다.)

재정의한 hashcode()는 인스턴스 생성시 필드값이 같다면,  

해시값(메모리 주소 해싱값)이 동일해지게 처리를 해줍니다.  
(원래 같은 경우 데이터가 같아도 메모리주소가 다르게 인스턴스가 생성됩니다.)

HashMap, HaseSet등의 자료구조는 내부적으로 객체를 비교할 때 

해당 객체의 해쉬값으로 비교를 하기 때문에 완벽하게 같은 데이터의 인스턴스로 다루고싶다면, 
...
- 위의건 데이터비교만 가능하게 해준 것이고, 아직 객체의 주솟값인 Hash 값이 다름 -> Hashcode를 사용하여 같게 맞춰준다.
- 결국 A인스턴스와 B인스턴스는 new해서 생성했지만 같은 데이터가 된다!


++ 객체를 System.out.println(); 했을때 나오는  
Main$객체@38db4c11 이라는 값은 메모리 주소값을 해싱하여 나온 값이다.
