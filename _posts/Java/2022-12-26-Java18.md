...
많기 때문애 이를 더욱 편하게 사용하고자 상속이 있습니다.
...
- ⭐️***자바의 모~든 클래스는 Object 클래스의 자식 클래스이다.***
- Object 클래스가 제공하는 메소드를 오버라이딩해서 메소드 기능을 변경하여 사용 가능하다.
...
인텔리제이에서는 equals 재정의와 hashcode를 맞추기 위한 기능을 제공합니다.  
...
재정의한 equals를 통해 필드값(데이터)을 비교하여 true & false를 리턴받게되고 , 
(원래같은 경우 아이덴티티 해쉬 코드 주소값만 비교합니다, 일반 해쉬코드 X)
...
위처럼 다른 인스턴스의 필드 타입, 필드 데이터가 같다면 같은 해쉬로 인스턴스를생성해줍니다.
(원래 같은 경우 데이터가 같아도 해쉬값이 다르게 인스턴스가 생성됩니다.)

### hashcode()까지 쓰는 이유.
사실 equals 까지만 재정의해도 보통의 상황에서는 객체끼리 데이터가 같은지 비교할 수 있습니다.

하지만, HashMap, HaseSet등의 자료구조는 내부적으로 객체를 비교할 때 

해당 객체의 해쉬값으로 비교를 하기 때문에 
Hash값을 사용하는 컬렉션을 사용할 때 !완벽하게 같은 데이터의 인스턴스로 다루고싶다면, 

hashcode()를 사용해서 주솟값 까지 맞춰주는 것입니다.

Hash값을 사용하는 컬렉션 은 객체가 논리적으로 같은지 비교할 때 거치는 과정

![](../../images/2022-12-30-17-02-43.png)

하지만 당연하게도 ==로 실제 객체의 내부참조 값은 identityHashCode로 동작하기 때문에
False가 나옵니다.

String의 경우 생성자를 사용하지 않고 똑같은 데이터로 초기화시,  
내부적으로 identityHashCode까지 똑같이 만들어놨기 때문에 == 해도 같은 주소로 나옴.

identityHashCode는 재정의가 불가능하여, 이 주소 마저도 같게 만드는 방법은 모르겠습니다...

아래의 글에서 보니identityHashCode는 소수의 확률로 중복이 될수 있다고합니다.
...
[참조](https://codingdog.tistory.com/entry/java-hashCode-vs-identityHashcode-%EC%9D%B4-%EB%91%98%EC%9D%80-%EB%AC%B4%EC%97%87%EC%9D%B4-%EB%8B%A4%EB%A5%BC%EA%B9%8C%EC%9A%94)
 ...
 정리를 해 봅시다. 
 identityHashCode에 비해서, 
 hashCode는 내가 오버라이딩을 할 수 있어요. 
 
 그렇지만 identity는 오버라이딩을 할 수 없어요. 
 
 즉, 재정의를 하지 못합니다. 

 만약에 오버라이딩이 되지 않았다면, 
 identity랑 해쉬코드 메서드의 리턴 값이 같다. 
 
 Hashcode를 오버라이딩 재정의 하는경우
identity값은 변경되어 -> 내부적으로 객체의 고유한 주소값을 똑같이 하는걸
막고 있는 것 같습니다.
...
아래 참조글을 보면 아마 자바에서는 
가비지컬렉터의 활용 때문에, 실제 주소값 자체를 활용하는건 의미가 없는 것 같습니다.  
[참조](https://okky.kr/articles/611110) 

[참조 블로그](https://gardeny.tistory.com/31)  
[해쉬값과 주소값 차이](https://okky.kr/articles/596050)  
[스택 오버플로우, 해쉬값 주소값 차이](https://stackoverflow.com/questions/3700595/difference-between-hash-code-and-reference-or-address-of-an-object)
...
상속의 장점은, 중복된 코드를 줄일 수 있고, 유지 보수가 편리하며, 통일성이 있고 다형성을 구현할 수 있습니다.

그리고 자바에서는 자식 클래스가 여러 부모로부터 다중 상속을 받는 것은 불가능합니다. 
...
근데 Cat은 Animal이면서 Cat이고 Dog은 Animal이면서 Dog이다.
...
추상이란 이름이 왜붙었을까? -> 추상 메소드를 가지고 있기 때문에  
자연스럽게 해당 클래스는 추상적인 클래스가 된다.

그럼 추상메소드는 왜 추상메소드인가

메소드의 시그니처만 존재하고 구현부는 없기 때문에 추상적임.  
이 추상메소드를 구현하면 구현한 해당 메소드는 비로소 실체 메소드가 된다.

이 추상 클래스는 말그대로 지금까지 작성해왔던  
구체적(concrete) 코드들과 다르기때문에 인스턴스화가 불! 가! 능! 하다

new 사용 시(인스턴스 생성 시) 추상 메소드를 오버라이딩 한다면,  
추상클래스가 추상메소드 때문에 추상적인 클래스인것이기 때문에

추상 메소드를 구현하면 추상적이지않은 구체적 클래스가 된다.  
그래서 사용이 가능해진다.

## 장점

추상화를 한다는것에 있어서의 장점은 무엇일까.

예를들어 동물 -> 강아지,고양이. 등의 동물은 먹기/걷기 등의 공통적인 행동을 하기 때문에

동물이라는 추상 클래스를 만들 수 있고 이뜻은 강아지,고양이... .등을 동물이라는 하나의 클래스로 추상화를 시킨것이다.

결국 Animal 타입으로 Cat, Dog, .. 클래스들을 다 받을수 있었던  
다형성과도 맞아 떨어지는 부분이다.

사실 이 예제만 봤을때는 위에서 상속부분에서 했던 예제와도 똑같다.
다만 동물 클래스를 상속받을 자식 클래스들에게 

어떠한 행동을 강제 하고 싶을때 추상클래스에 추상메소드를 넣어주면된다.  
강제하고 싶다는건? 해당 자식클래스들의 공통적인 메소드라는 뜻이다.
...
그럼 이 메서드를 추출해서 추상클래스 안에 두면 된다. 

(키보드를 상속받아 탄생한,  A키보드, B키보드, C키보드)
이때 이들 키보들간의 공통적인 필드
키보드 이름, 가격 등을 추상클래스에 넣으면된다.
