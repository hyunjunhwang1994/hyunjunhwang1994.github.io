...
hashcode()를 오버라이딩해 해쉬값까지 맞추어 주어야 합니다.
...
- 하지만 "=="로 실제 객체의 내부참조 값은 identityHashCode로 동작하기 때문에
False가 나옵니다.
- String의 경우 new를 사용하지 않고, 같은 값으로 초기화 한다면,
...
그냥 단순히 부모 클래스에서 만든 메소드를 자식 클래스가 오버라이딩 해도 되지만,
추상메소드를 사용하지 않는다면 해당 오버라이딩을 강제시키진 못한다.
